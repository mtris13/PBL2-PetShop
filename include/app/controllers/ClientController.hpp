...
private:
  ...
  void displayDogsForSale() {
    ...
    while (current != nullptr) {
      cout << left << setw(6) << current->getData().getId() << setw(15)
           << current->getData().getName() << setw(20)
           << current->getData().getBreed() << right << setw(6)
           << current->getData().getAge() << setw(14) << fixed
           << setprecision(0) << current->getData().getPrice() << setw(7)
           << current->getData().getEnergyLevel() << "/10" << setw(12)
  ...
  void checkout() {
    ...
    LinkedList<CartItem> cartItems =
        cartRepo.getCartItems(currentClient->getId());
    ...
    while (item != nullptr) {
      billItems.pushBack(BillItem(item->getData().getPetId(),
                                  item->getData().getPetName(), "Pet",
